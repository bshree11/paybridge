...
class FraudDetectionService {
  ...
  private async checkVelocitySpike(
    data: TransactionData,
  ): Promise<FraudRuleResult> {
  ...
  private async checkNewAccount(
    ...
    const accountAge =
      Date.now() - new Date(result.rows[0].created_at).getTime();
  ...
  private async checkCrossBorder(
    data: TransactionData,
  ): Promise<FraudRuleResult> {
    const result = await pool.query(
  ...
  private async checkRoundNumber(
    data: TransactionData,
  ): Promise<FraudRuleResult> {
    const amountGBP = await this.convertToGBP(data.amount, data.currency);
  ...
  private async checkNightTransaction(
    data: TransactionData,
  ): Promise<FraudRuleResult> {
  ...
  private async convertToGBP(
    ...
    const fallbackRates: Record<string, number> = {
      USD: 0.74, // 1 USD = 0.74 GBP
      EUR: 0.86, // 1 EUR = 0.86 GBP
      INR: 0.0078, // 1 INR = 0.0078 GBP
    };
